...
static void sha256_transform(uint32_t *state, const void *_input)
{
...
{
    uint32_t *dst = _dst;
    unsigned int i, partial = s->count & 0x3f;
...
static void __init __constructor test_sha256(void)
{
    for ( unsigned int i = 0; i < ARRAY_SIZE(tests); ++i )
    {
        ...
        sha256_digest(res, t->msg, strlen(t->msg));
